...
func text(stopchan chan bool, exitchan chan int, i int) {
	...
	for {
		select {
            case <-stopchan: // stopchan是一个无缓存的信道,只有在信道被关闭后(false),才能走这条分支
...
func main() {
	exitchan := make(chan int, 5) // 实现主线程和协程间的同步退出
	stopchan := make(chan bool)
	for i := 1; i <= 5; i++ {
		go text(stopchan, exitchan, i)
	}
	time.Sleep(time.Second)
	close(stopchan) // 关闭stop信道
	for i := 1; i <= 5; i++ {
		<-exitchan
	}
}
...
func main() {
    // 第二个参数为绝对时间,当到达这个时间,自动调用cancel()函数结束协程.
	...
	time.Sleep(time.Second)
    cancel() // 此函数是不起作用的,因为0.5秒后会自动调用封装好的chancel()函数
	fmt.Println(ctx.Err()) // 打印cancel原因
	// 退出
	for i := 1; i <= 5; i++ {
		<-exitchan
	}
...
WithTimeout和WithDeadline功能基本相同,唯一不同的是,WithTimeout第二个参数是一个相对时间,就不举例说明了.
